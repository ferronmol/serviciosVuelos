...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    //si recibo un idpasaje pero no un identificador
    if (isset($_GET['idpasaje']) && !isset($_GET['identificador'])) {
    ...
    // si recibo un identificador pero no un idpasaje
    if (isset($_GET['identificador']) && !isset($_GET['idpasaje'])) {
        $res = $booking->getObjectBooking($_GET['identificador']);
        //si el pasaje no existe
        if ($res == null) {
            //devuelvo un error
            http_response_code(404);
